// FRONTAGE — San Francisco search, from DataSF's Assessor secured-roll
...
import { clean, toNum, sqlStr, socrata } from "../_lib/util.js";

const SF_BASE = "https://data.sfgov.org";
...
export async function search(q) {
  const { address, propertyType, minValue, maxValue, minSqft, limit } = q;
  // Unified vocabulary: `town` doubles as the neighborhood in SF.
  const neighborhood = q.neighborhood ?? q.town;
  ...
  const rows = await socrata(SF_BASE + "/resource", SF_ROLL, {
  ...
  return {
    market: "sf", count: out.length, neighborhood: clean(neighborhood) || null, roll_year: ROLL_YEAR,
