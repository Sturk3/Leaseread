...
const TOOLS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        targetsOnly: { type: "boolean", description: "Optional HARD filter that removes non-acquirable owners entirely. Normally leave it OFF: by default every lead is instead LABELLED with sale_likelihood ('none' for public / institutional / co-op owners, with a sale_note explaining why) and stays in the results — the user wants to see the whole block and know which rows can't be bought. Set true only if they ask to hide them." },
  ...
  },
  {
    ...
    description:
      ...
      "SHORT-TERM-RENTAL + ACCOMMODATIONS overlay membership, FEMA FLOOD zone + SFHA + BFE, STREET-FLOODING history within " +
      "¼ mi (city flooded-vehicle log — Charleston's chronic-flooding signal FEMA maps miss), and CRIME within ~300m (last 3 " +
      "years, by category). Use after search_charleston_properties for diligence/constraints/motivation. City layers only cover " +
      "the City of Charleston; Mt Pleasant / N. Charleston parcels get county zoning + FEMA + parcel facts.",
    input_schema: {
      type: "object",
      properties: {
        pid: { type: "string", description: "Parcel PID from a search_charleston_properties result (e.g. '4570402030'). Preferred — records join exactly on it." },
        address: { type: "string", description: "Street address (resolved to the parcel if no PID)." },
      },
    },
  },
  {
  ...
  {
    name: "sc_entity_lookup",
    description:
      "Unmask a SOUTH CAROLINA business entity / LLC (the Charleston analog of the NY/CT/CA/TN registry lookup). Given an " +
      "LLC/Corp name, reads the SC Secretary of State Business Entities Online registry (businessfilings.sc.gov) and " +
      "OpenCorporates and returns: exact entity name, type, status (good standing/dissolved), filing date, the REGISTERED " +
      "AGENT (name + address — the key contact for an anonymous LLC), the principal/mailing address, and any listed " +
      "officers/members. Use for Charleston/SC owners. NOTE: South Carolina gates its registry search behind a captcha (no " +
      "open API), so this is a live WEB lookup (metered like web_research) — call it once per entity.",
    input_schema: {
      type: "object",
      properties: { name: { type: "string", description: "South Carolina entity / LLC name, e.g. '806 MERIDIAN LLC'." } },
      required: ["name"],
    },
  },
  {
...
- SELLABILITY IS A LABEL, NOT A FILTER: leads carry sale_likelihood — "none" means the owner effectively never sells (university/church/hospital, city/state/authority, or a co-op/condo corporation with no single seller), with sale_note giving the reason. Keep those rows in the roster but mark them clearly (e.g. "⛔ NYU — institutional, zero chance of selling") and rank them LAST, below every private owner. Never present one as a target, and never silently drop it either.
- SIZE RANGES ARE ABSOLUTE: if the user gives a square-footage range, every row must be inside it. Don't include a near-miss "for reference" — if you think a just-outside building matters, say so in one line after the roster.
- EVERY ROW READS AS A TARGET: lead each candidate with the number the user asked about. For a size-based ask that's the relevant AREA — "**39 W 19th St** — 9,500 SF office · Abner Properties Co (held 56 yrs, same family entity)" — then the signals and the contact path. Never make the user open a spreadsheet to find out how big a building is.
...
- web_research / web_search / ca_entity_lookup / brand_radar hit the live web and are the EXPENSIVE part of an answer (a deep web_research run costs real money — treat each one like a paid report, not a free lookup). Use them where they genuinely change the answer — unmasking an LLC, finding principals/contacts, distress narrative, non-NYC markets — but budget them: normally ONE web_research call per question, TWO at most, with everything you need batched into that one query. In an AREA SWEEP, build the whole roster from the FREE engines and spend web calls only on the 1–3 top targets (or when the user explicitly asks for contacts across the list). Never repeat a call you've already made this conversation.
...
3) USE THE WEB DELIBERATELY. In deep mode you may make SEVERAL web calls (web_research / ca_entity_lookup / web_search) — this is the mode where the user opted into spending — but each one is expensive, so make each one count: batch related questions, never repeat a call, and stop when the marginal call wouldn't change the report.
