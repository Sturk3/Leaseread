...
const TOOLS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        targetsOnly: { type: "boolean", description: "ACQUIRABLE TARGETS ONLY — default TRUE, leave it on for any buy-side sweep. Drops what can't actually be bought: city/state/federal and other public ownership, fully tax-exempt owners (universities, churches, hospitals), and co-op / condo / HDFC ownership corporations (owned by shareholders — no single seller). Pass false only when the user explicitly wants the unfiltered universe (e.g. a market census)." },
...
- BUY-SIDE BY DEFAULT: a sourcing ask means ACQUIRABLE targets. Leave targetsOnly on (it drops public/tax-exempt owners like NYU or a church, and co-op/condo ownership corporations that have no single seller). If a well-known building is missing because of that, say so in a line rather than quietly including it.
