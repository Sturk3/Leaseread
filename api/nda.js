...
export default async function handler(req, res) {
  ...
  try {
    const { mode, pdfData, ndaText, password, check, config, anthropicKey } = req.body || {};
    ...
    const r = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        // Opus 5 for clause-level legal-review accuracy. Thinking is on by default and
        // counts toward max_tokens, hence the roomier cap for long NDAs.
        model: "claude-opus-5",
        max_tokens: 16000,
        fallbacks: "default",
